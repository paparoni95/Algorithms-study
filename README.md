...
## 8. Tree (트리)
비선형 구조이며, 원소들 간의 `1 : n` 관계를 가지는 자료구조이다. 다른 말로 `계층형 자료구조`라고도 한다. <br>
...
명칭 | 설명
---|---
노드(Node) | 자료를 보관하는 단위, 정점(Vertex)로 불리기도 함
간선(Edge) | 노드에서 다른 노드로 이동하는 경로, 가지(Branch)로 불리기도 함
루트노드(Root Node) | 부모가 없는 노드, 트리는 단 하나의 루트 노드만 가짐
단말노드(Terminal Node) | 자식이 없는 노드, 말단 노드 혹은 잎 노드로 불린다.
부모노드(Parent Node) | 자식의 부모가 부모노드
자식노드(Child Node) | 부모의 자식들이 자식노드
조상노드(Ancestors) | 자신에게 오기 위한 경로에 있는 모든 노드들
...
### 8.2. Binary Tree (이진 트리)
1. 모든 노드들이 2개의 서브트리를 갖는 특별한 형태의 Tree
2. 한 노드가 최대 2개의 자식만을 가질 수 있는 형태
3. 레벨 i단계에서 노드의 최대 개수는 `2^i`개
4. 높이가 `h`인 이진 트리에서 가질 수 있는 최소 노드의 개수는 `h+1`개이고, 최대는 `2^(h+1) - 1`개 이다.

* 이진 트리의 구조는 3개
	- 포화 이진트리 (Full Binary Tree) : 모든 레벨에 노드가 포화상태 (최대의 노드 개수를 가짐)
	- 완전 이진트리 (Complete Binary Tree) : 마지막 레벨을 제외한 각 레벨이 포화여야 하며, 마지막 레벨은 왼쪽부터 채워져야 한다.
	- 편향 이진트리 (Skewed Binary Tree) : 높이 h에 대한 최소 개수의 노드를 가지면서 한쪽 방향으로 자식 노드만을 가진 트리

![Trees](http://ehpub.co.kr/wp-content/uploads/2016/06/3-50.png) <br>
...
### 1.3. getline 함수
대부분 문자열을 입력을 받을 때, 보통은 `love`와 같이 공백이 없는 문자열은 `scanf()`나 `cin`와 같은 형태로 문자열을 입력받는다.
그런데 공백이 있는 문자열의 경우 `Hello World!`는 `getline`을 사용해서 입력받는다.
C언어와 C++언어의 두 가지 종류 getline에 대해서 알아보자.

**C getline()** <br>
`#include <cstring>` <br>
`istream& getline(char * s, streamsize n);` <br>
`istream& getline(char * s, streamsize n, char delim);` <br>
1. `char * s` 는 C형식 스타일의 문자열을 저장할 배열을 가리키는 포인터
2. `streamsize n`은 저장할 문자의 최대 개수 (중요한 점은 문자열의 끝을 가리키는 널 문자를 포함한 값)
3. `char delim`은 제한자로서, 이 문자에 도달하게 되면 추출이 중단된다. (별도의 지정이 없다면 `\n`으로 인식한다.)
중요한 점은 문자열에 C배열 타입말고 string으로 넘겨버리면 error가 발생한다. 따라서 string은 string 라이브러리의 getline함수를 사용하자. 추가적으로 설명하면 C언어에서의 string을 사용하려면 헤더파일을 위와 같이 사용해야 한다.

**C++ getline()** <br>
`#include <string>` <br>
`istream& getline(istream& is, string& str);` <br>
`istream& getline(istream& is, string& str, char delim);` <br>
1. `istream& is`는 입력스트림 오브젝트 (보통 표준입력인 cin)
2. `string& str`은 입력받을 문자열을 저장할 string 객체
3. `char delim`은 제한자의 역할을 한다.
C++ 스타일의 문자열로 사용하고 싶다면 헤더파일을 위와 같이 사용해야한다.

### 1.4. cin과 getline의 차이점
일반적으로 `cin`은 입력값의 기준을 `띄어쓰기, 엔터, 탭`으로 나누게 된다. 그래서 문자를 읽다가 해당 기준을 만나게 되면 그 전까지의 입력들을 하나의 입력으로 받아들이고 저장하게 된다. 반면에 해당 입력값의 기준이 먼저나오게 되면 그냥 무시해버리고, 그 대신에 버퍼에 그대로 남아있게 된다. 또한 `cin`은 버퍼에 있는 글자 하나하나를 읽으며, 내부적으로는 공백 같은 문자들을 무시해주는 `formatting`함수가 있다.
<br><br>
`getline`은 반대로 하나의 입력으로 받아들이는 기준을 `delim`으로 지정해주지 않는 한 `\n`이다. 이 말은 엔터를 무시하지 않고 잡는다는 뜻으로 엔터 직전까지의 입력들을 하나의 입력으로 받아들이고 엔터를 버리게 된다. (버퍼에 남겨두지 않는다. 아까 위에서 봤던 `cin`은 엔터를 버리지 않고 버퍼에 남긴다.) 내부적으로는 `unformatting`함수가 있다.
<br><br>
차이점을 정리하면 다음과 같다.
1. `cin`은 엔터를 무시하지 않고 버퍼에 남겨둔다.
2. `getline`은 엔터를 무시하는 기능은 없고 catch한다.

### 1.5. scanf함수의 비밀
scanf함수도 반환값이 존재한다. 여기서의 반환값은 `첫 번째 인자에 해당하는 포맷 형식의 개수를 반환`한다. 이 말은 성공적으로 받아온 수를 기준으로 return을 의미하며, 만약에 에러가 발생하거나 `EOF(파일의 끝)`을 만나면 `-1`을 return하게 된다. 또한 `엔터, 탭, 스페이스`의 공백을 문자열로 처리하지 않는다.
...
while(~scanf("%d", &n))
```
위의 3가지 코드는 모두 입력이 끝날 때까지 입력받는 방법이다. 마지막의 `~`은 입력받은 값이 없다면 -1을 scanf함수에서 반환하게 되는데, 비트연산자 `~`에 의해서 `0`이 되므로 결국에는 거짓을 반환해서 종료하는 것이다.
